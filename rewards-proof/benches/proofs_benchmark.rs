...
use bulletproofs::{BulletproofGens, LinearProof, PedersenGens, RangeProof};
...
use rewards_proof::api::{
    linear_proof, linear_verify, linear_verify_multiple, range_proof, range_verify,
    range_verify_multiple, setup,
};
...
fn benchmark_rewardsproof_generation(c: &mut Criterion) {
    ...
    for size in [64, 128, 256].iter() {
        //pre-processing
        let private_value: Vec<_> = (0..*size as usize)
            .map(|_| Scalar::random(&mut rng))
            .collect();
        let public_value: Vec<Scalar> = (0..*size as usize)
            .map(|_| Scalar::random(&mut rng))
            .collect();
...
fn benchmark_rewardsproof_verification(c: &mut Criterion) {
    ...
    for size in [64, 128, 256].iter() {
        ...
        // linear proof
        let (linear_proof, linear_commitments) = linear_proof(
            &ps_gen_lin,
            &bp_gen_lin,
            private_value.clone(),
            public_value.clone(),
        ...
        group.bench_with_input(BenchmarkId::new("linearproof-", size), size, |b, _size| {
            b.iter(|| {
                linear_verify(
                    linear_proof.clone(),
                    public_value.clone(),
                    linear_commitments.0.clone(),
                    linear_commitments.1,
                    linear_commitments.2,
                    linear_commitments.3,
                )
            })
        });
    }
...
fn benchmark_rewardsproof_verification_multiple_users(
    ...
    let mut linear_proof_commitments: Vec<(
        Vec<RistrettoPoint>,
        RistrettoPoint,
        RistrettoPoint,
        CompressedRistretto,
    ...
    for _x in 0..number_of_users {
        ...
        //pre-processing
        let private_value: Vec<_> = (0..incentive_size)
            .map(|_| Scalar::random(&mut rng))
            .collect();
        let public_value: Vec<Scalar> = (0..incentive_size)
            .map(|_| Scalar::random(&mut rng))
            .collect();
    ...
    // Verify proofs
    group.bench_function("multiple_range_proofs", |b| {
        b.iter(|| {
            range_verify_multiple(
                ps_generators.clone(),
                bp_generators.clone(),
                range_proofs.clone(),
    ...
    group.bench_function("multiple_linear_proofs", |b| {
        b.iter(|| {
            linear_verify_multiple(
                linear_proofs.clone(),
                public_values.clone(),
